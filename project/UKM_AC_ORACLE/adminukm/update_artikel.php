                                ...
                                <div class="col-md-6">
                                    <form role="form" method="POST" enctype="multipart/form-data" action="">
                                        <div class="form-group">
                                            ...
                                            <input class="form-control" type="text" value="<?php echo $row->JUDUL; ?>" placeholder="Masukkan Judul" name="judul" />
                                        </div>
                                        <div class="form-group">
                                            <label>Pilih Gambar</label>
                                            <input type="file" name="gambarArtikel"/><p class="help-block">Tipe file harus : jpeg/jpg/png</p>
                                            <input type="hidden" name="gambarArtikel2" value="<?php echo $row->GAMBAR_POSTING; ?>"/>
                                        </div>
                                        <div class="form-group">
                                            <label>Isi Artikel</label>
                                            <textarea class="form-control" name="isiArtikel" rows="20"><?php echo $row->ISI; ?></textarea>
                                        </div>
...
<?php
	...
	if(isset($_POST['upload'])){
		...
		$isi2 = nl2br($isi);
		$tgl_up = date("Y-m-d");
		$foto = $_POST['gambarArtikel2'];
		$lihat = $row['view'];
		$idukm = $row['id_ukm'];
		
		$nama_file = $_FILES['gambarArtikel']['name'];
        $ukuran_file = $_FILES['gambarArtikel']['size'];
        $tipe_file = $_FILES['gambarArtikel']['type'];
        $tmp_file = $_FILES['gambarArtikel']['tmp_name'];
            
        $waktu = date('His');
        $nama_file_baru = $waktu.$nama_file;
        $path = "assets/img/Artikel/".$nama_file_baru;
		
		if($tipe_file == "image/jpeg" || $tipe_file == "image/jpeg" || $tipe_file == "image/png"){
			if($ukuran_file <= 2000000){
				move_uploaded_file($tmp_file, $path);
             
				$sql = "INSERT INTO posting VALUES ('', '$judul', '$nama_file_baru', '$tgl_up', '$isi2', '', '', '', '', 'Artikel', $lihat, '$idukm')";
				$read = oci_parse($conn, $sql);
                oci_execute($read);
                if($read==true){
			...
			$sql = "INSERT INTO posting VALUES ('', '$judul', '$foto', '$tgl_up', '$isi2', '', '', '', '', 'Artikel', $lihat, '$idukm')";
				$read = oci_parse($conn, $sql);
                oci_execute($read);
                if($read==true){
					echo "<font color='blue'>Data berhasil diupdate</font>";
